						...
						<table id="dynamic-table" class="table table-striped table-bordered table-hover">
							<thead>
								<tr>
									...
									<th>Tiêu đề</th>
									<th class="hidden-480">Thời gian thử việc</th>
							...
							<?php if(!empty($list)){?>
								<?php $i=0; foreach ($list as  $value) {$i++;?>
									...
									<td>
										<a href="<?php echo base_url('jop/cong_ty/'.$info[$i-1]->id); ?>"><?php if(!$info){echo 'Không tồn tại';}else{echo $info[$i-1]->sComName;} ?></a>
									</td>
									<td>
										<a href="<?php echo base_url('jop/cong_viec/'.$value->id); ?>"><?php echo  $value->sJobTitle; ?></a>
									</td>
								
									<td ><?php if(!$info_wage[$i-1]){ echo 'Không tồn tại';}else{ echo $info_wage[$i-1]->sProbationaryName;} ?></td>
